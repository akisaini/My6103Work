...
fig.suptitle('Chart representing class, age, gender and surival')
axes[0].set_title('Step wise bar: Ticket class vs Gender')
axes[0].hist(men_titanic['pclass'], density=True, histtype='step', stacked=True)
axes[0].hist(female_titanic['pclass'], density=True, histtype='step', stacked=True)
axes[0].legend(titanic['sex'])
axes[0].set_xlabel('Ticket Class')

axes[1] = sns.boxplot(x = titanic['age'], y = titanic['sex'], hue = titanic['survived'], palette = 'Set3')
...
# From the first subplot, we can observe that there are more male passengers in 3rd class than female passengers. And more female passengers in first class and second class than male passengers.  

# From the second subplot, we can observe the average age of the passengers, both men and women, and the age statistics of both survived and dead passengers. 
# Female and male passengers who survived the incident were on average older than the ones that did not survive. Overall, female passengers were much younger than male passengers. 
...
homewin_counts.plot(kind = 'bar', color = 'purple')
plt.show()
...
homeloss_counts.plot(kind = 'bar', color = 'red')
plt.show()
...
# Dropping columns that do not affect the target variable like the 'name', individual team names, 'gamedate', and 'season' column:

X = X.drop(['name','AwayTeam','HomeTeam','kickteam','GameDate','def','season'], axis = 1)
...
nflmodel = LogisticRegression(max_iter= 1000)

nflmodel.fit(X_train, y_train)

y_predict = nflmodel.predict(X_test) 

nflmodel.score(X_test, y_test)

cm_nfl_model = confusion_matrix(y_test, y_predict)

sns.heatmap(cm_nfl_model, annot=True, fmt="d")
plt.show()

# 32 times the kick was not good and the model predicted it to be not good. 
# 0 times the kick was good and the model predicted it to be not good. 
# 176 times the kick was good and the model predicted it to be good. 
# 0 times the kick was not good and the model predicted it to be good.

print(classification_report(y_test, y_predict))

# 100% accuracy anf f1 score in the model. 
#%%
# 
# ## Question 7  
...
# According to the models test set, out of 208 times, just 32 times the kick was missed. So Field goals are usually good irrespective of whether the team is playing at home or away. 

# For instance, lets pick a random team: CLE where they awere playing at home and check their missed vs good field goals.

cle_miss_home = nfl[(nfl['HomeTeam'] == 'CLE' ) & ( nfl['Missed'] == 1)]
cle_good_home = nfl[(nfl['HomeTeam'] == 'CLE' ) & ( nfl['Missed'] == 0)]

# CLE missed 7 times as compared to having 25 good field goals.

plt.boxplot(cle_miss_home['distance'])
plt.show()
# At home games, the missed field goal usually lies in the range of 38 to 50. 
plt.boxplot(cle_good_home['distance'])
plt.show()
# At home games, the good field goal usually lies in the range of 30 to 36. 
...
# Continuing with CLE, lets count the missed and good field goals while they were playing away:

cle_miss_away = nfl[(nfl['AwayTeam'] == 'CLE' ) & ( nfl['Missed'] == 1)]
cle_good_away = nfl[(nfl['AwayTeam'] == 'CLE' ) & ( nfl['Missed'] == 0)]

# At road games CLE missed 6 field goals compared to 26 good field goals. These are approximately the same as home game numbers. 

plt.boxplot(cle_miss_away['distance'])
plt.show()
# At away games, the missed field goal usually lies in the range of 45 to 52. 
plt.boxplot(cle_good_away['distance'])
plt.show()
#  At away games, the good field goal usually lies in the range of 30 to 42. 

# Conclusion: The numbers for CLE were approximately the same for home and road games. Home teams and Road teams usually have similar chances of making a successful field goal. 
